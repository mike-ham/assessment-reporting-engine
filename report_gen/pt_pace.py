...
def generate_pace_document(output, json, assets):
    ...
    purpose_description = Paragraph('<font size="10" color="#5A5B5C">As a follow up to the security assessment performed for '+stakeholder_name+' approximately ninety (90) days ago, the security team is requesting feedback regarding the status of mitigation and remediation efforts. These responses will help the security team measure and validate the effectiveness of the recommendations and mitigations supplied within the assessment report. Additionally, the responses will help identify opportunities for enhancing the security team''s services.</font>', body_style)

    instructions = Paragraph('<font size="14" color="#5A5B5C">Instructions</font>', header_style)
    instructions_1 = Paragraph('<font size="10" color="#5A5B5C">1.  The Assessment Information section should be completed by a organizational representative, then reviewed and validated by '+stakeholder_name+'.</font>', body_style)
    instructions_2 = Paragraph('<font size="10" color="#5A5B5C">2.  The Mitigated Findings section should be completed by a organizational representative in coordination with a representative from '+stakeholder_name+' by selecting the most accurate mitigation status, action, and/or challenge preventing full remediation for each finding.</font>', body_style)
    instructions_3 = Paragraph('<font size="10" color="#5A5B5C">3.  The Risk Score section provides the Total and Mitigated Risk Scores that were calculated during the assessment timeframe. The 90-Day Mitigated Risk Score will be determined by a organizational representative based on the responses received within the Mitigated Findings section</font>', body_style)
    ...
    risk_description = Paragraph('<font size="10" color="#5A5B5C">From the findings, a Total Risk Score was calculated to measure progress as findings are mitigated. If a finding was mitigated during the assessment timeframe and the security team was able to validate mitigation, the Mitigated Risk Score represents the deduction of those mitigated findings. Based on the responses to this questionnaire, an updated 90-Day Risk Score will be calculated.</font>', body_style)
    ...
    risk_end = Paragraph('<font size="10" color="#5A5B5C">The security team identified '+num.number_to_words(severity_count['Critical'])+' ('+str(severity_count['Critical'])+') critical findings, '+num.number_to_words(severity_count['High'])+' ('+str(severity_count['High'])+') high findings, '+num.number_to_words(severity_count['Medium'])+' ('+str(severity_count['Medium'])+') medium findings, '+num.number_to_words(severity_count['Low'])+' ('+str(severity_count['Low'])+') low findings, '+num.number_to_words(severity_count['Informational'])+' ('+str(severity_count['Informational'])+') informational findings. The Total Risk Score for this assessment is '+str(total_risk_score)+' and the Mitigated Risk Score after deducting mitigated findings is '+str(mitigated_risk_score)+'. The 90-Day Risk Score will be calculated once this evaluation is completed.</font>', body_style)
